...
const QUESTION_DETAILS_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vRJl8XYak_fAzpuboA6GgOO-hEMd6rP_X9BD7ruZ-pSnIGKkd27uGmP2ZWeBcwSvSKsafObcXDOW080/pub?gid=822014112&single=true&output=csv';
...
// --- CONFIGURATION ---
// IMPORTANT: Change this to the student EMAIL you want to view
const STUDENT_EMAIL_TO_FILTER = "student@example.com"; // Use this email ID from your data
...
async function loadAndProcessData() {
    ...
    try {
        ...
        // --- Step 1: Filter data for the specific student using EMAIL ---
        const studentInfo = studentMappingRaw.find(s => s['Student Email'] === STUDENT_EMAIL_TO_FILTER);
        if (!studentInfo) {
            console.error(`Student with email ${STUDENT_EMAIL_TO_FILTER} not found in student mapping.`);
            document.getElementById('overview-kpis').innerHTML = '<p class="text-red-600 text-center col-span-5">Error: Student data not found. Please check the STUDENT_EMAIL_TO_FILTER.</p>';
            return; // Stop execution if student not found
        }
        const studentIDFromMapping = studentInfo['Student ID']; // Get the actual Student ID from the mapping
        ...
        console.log(`Filtered data for student ${studentIDFromMapping} (Email: ${STUDENT_EMAIL_TO_FILTER}):`);
...
// --- Main Execution Block (initiated by inline script in index.html) ---
